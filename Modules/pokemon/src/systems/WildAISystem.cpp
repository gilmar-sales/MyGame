...
namespace {
    ...
    void PlayLoco(fr::Registry &registry, const skr::Arc<fg::AnimationController> &animation,
        ...
        fr::Entity animator = fr::NullEntity;
        ...
        if (animator == fr::NullEntity) {
            ...
            ai.animatorEntity =
                    animator != fr::NullEntity ? static_cast<std::int64_t>(animator) : -1;
        }
        if (animator != fr::NullEntity) {
...
void WildAISystem::Update(float deltaTime) {
...

    {
        ...
        if (mPlayer == fr::NullEntity || !mRegistry->HasComponent<PlayerTag>(mPlayer)) {
            const auto player = mRegistry->CreateQuery()->First<PlayerTag>();
            mPlayer = player.has_value() ? player.value() : fr::NullEntity;
        }
    }

    glm::vec3 playerPos{};
    bool playerAlive = false;
    if (mPlayer != fr::NullEntity &&
        ...
        mPlayer = fr::NullEntity;
    }

    const bool hasPlayer = playerAlive && mPlayer != fr::NullEntity;
            ...
            .EachAsync(
                [&](fr::Entity entity, WildPokemonAI &ai, PokemonVitals &vitals, PokemonMoveset &moves,
                    ...
                    const glm::vec3 toPlayer =
                            hasPlayer
                                ? glm::vec3{playerPos.x - pos.x, 0.0f, playerPos.z - pos.z}
                                : glm::vec3{};
...

                    {
                        ...
                        const glm::vec3 wanderVel{
                            ai.wanderDirX * ai.moveSpeed, 0.0f,
